...
class DanteMorningParams:
    ...
    """
    # A는 단일표본 집계 +4.72%p로 최고였으나, 견고성검증(다른 종목군)서 후반 엣지 -1.66%p로
    # 뒤집힘 → 표본 의존(불안정). vol 2.5·trend60 튜닝도 robust 미입증이라 단테 충실값으로 환원.
    # (등락률 2~10 캡은 +0.69%p로 급락 = 10~15% 급등주가 승자이므로 max는 15+ 유지가 맞음)
    vol_prev_mult: float = 2.0      # 전일대비 거래량 ≥ 100%(=2배, 단테 원값)
